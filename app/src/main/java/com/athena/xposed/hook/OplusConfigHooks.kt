 ...
 *     [XmlPolicyBuilder.buildEnhancedXml] 注入 whitePkg / ffPkg / imPkg 与超时
 *     覆盖后返回，从而在不修改磁盘文件的前提下动态改写冻结策略。
 *
...
object OplusConfigHooks {
    ...
    fun install(
        ...
        installOplusSettingsReadConfig(module, engine, classLoader, handles)
        installElsaConfigFileInputStream(module, engine, handles)
    }
            ...
            .intercept { chain ->
                ...
                try {
                    ...
                    if (fileName == AUTOSTART_WHITELIST_FILE && result is String) {
                        val policy = engine.getEffectiveConfig()
                        if (policy.whitePkg.isEmpty()) {
                            // 白名单为空时无需改写，避免无谓字符串拼接
                            return@intercept result
                        }
                        val enhanced = XmlPolicyBuilder.buildEnhancedAutoStartWhiteList(
                            originalContent = result,
                            whitePkgList = policy.whitePkg
                        )
                        module.log(
                            Log.DEBUG, TAG,
                            "readConfig($fileName) injected ${policy.whitePkg.size} white pkgs."
                        )
            ...
            val h = module.hook(ctor)
                ...
                .intercept { chain ->
                    ...
                    try {
                        ...
                        hijackStream(module, engine, fis)
                    } catch (t: Throwable) {
    ...
    private fun hijackStream(
        ...
        val policy: FreezePolicy = try {
            engine.getEffectiveConfig()
        } catch (t: Throwable) {
            module.log(Log.WARN, TAG, "getEffectiveConfig failed, skip hijack.", t)
            return
        }

        // 白名单 / 黑名单 / IM 全空且超时为默认时，无需改写
        if (policy.whitePkg.isEmpty() && policy.ffPkg.isEmpty() && policy.imPkg.isEmpty()) {
            module.log(Log.DEBUG, TAG, "Policy empty, skip elsa hijack.")
            return
        }

        val enhancedXml = XmlPolicyBuilder.buildEnhancedXml(originalXml, policy)
        ...
        module.log(
            Log.INFO, TAG,
            "Elsa config hijacked: original=${originalBytes.size}B enhanced=${enhancedBytes.size}B policy=${policy.signature}"
        )
    }
