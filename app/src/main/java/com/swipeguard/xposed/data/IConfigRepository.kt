...
interface IConfigRepository {
    ...
    /**
     * 加载当前配置。反序列化失败时实现应回退到 [SwipeGuardConfig.DEFAULT]，
     * **不得**抛出异常导致宿主进程崩溃。
     */
    fun load(): SwipeGuardConfig
     ...
     */
    fun observeChanges(listener: (SwipeGuardConfig) -> Unit)

    /** 注销通过 [observeChanges] 注册的监听器。重复注销或注销未注册的监听器为 no-op。 */
    fun removeOnChangeListener(listener: (SwipeGuardConfig) -> Unit)
